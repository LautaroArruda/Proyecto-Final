...
def ingresarAlumnos():
    try:
        ...
        genero = input("Ingresar género (Masculino / Femenino / Transgénero / No binario): ").capitalize() # capitalize pone la primera letra en mayúscula
...
def modificarAlumno():
    ...
    try:
        ...
        resultado = cursor.fetchone() # Obtiene sÓlo el primer registro de la consulta como tuplas (no son modificables)
        ...
        curso_id = input("Ingrese el número del curso que pertenece (1-6): ").strip()
        ...
        valores = (nombre.title(), apellido.title(), dni, direccion.title(), genero,
                   fechaNacimiento, email.lower(), curso_id)
...
def mostrarDatosAlumnos():
    try:
        ...
        cursor.execute(consulta) # Ejecutar la consulta con el parámetro seguro (con tuplas y no como caracteres)
...
def mostrarAlumnosOrdenados():
    try:
        ...
        for apellido, nombre, dni, legajo, direccion, cursos_idCurso in alumnos:
            print(f"Apellido: {apellido} , Nombre: {nombre}, DNI: {dni}, Legajo: {legajo}, Dirección: {direccion}, Curso: {cursos_idCurso}")
            print("-"*200)
...
def mostrarProfesoresOrdenados():
    try:
        ...
        """ 
        cursor.execute(consulta) 
        profesores = cursor.fetchall() # Obtiene todos los registros de la consulta como tuplas no modificables
        ...
        for apellido, nombre, dni, matricula, telefono, direccion in profesores:
            print(f"Apellido: {apellido}, Nombre: {nombre}, DNI: {dni}, Matrícula: {matricula}, Teléfono: {telefono}, Dirección: {direccion}")
            print("-"*200)
    except Exception as e: 
        print(f"Ocurrió un error durante la consulta: {e}")
    finally:
...
def buscarEspecificoProfesor(dato, comandoSQL):
    ...
    try:
        ...
        cursor.execute(query, (dato,)) # Ejecutar la consulta con el parámetro seguro (con tuplas (no modificables) y no como caracteres)
        resultados = cursor.fetchall() # Obtiene todos los registros de la consulta como tuplas no modificables
    ...
    try:
        ...
        if resultados:
            ...
            for legajo, nombre, apellido, email, direccion, genero in resultados:
                print(f"Legajo: {legajo}, Nombre: {nombre}, Apellido: {apellido}, Email: {email}, Dirección: {direccion}, Género: {genero}") # f-strings permite insertar los valores de las variables dentro de la cadena directamente, de forma legible.
        else:
            print(f"No se encontraron registros con: {dato}")
...
def listaAlumnos():
    try:
        ...
        cursor.execute(sql)
        resultado = cursor.fetchall() # Obtiene todos los registros de la consulta como tuplas no modificables
        if not resultado:
...
def eliminarProfe(dato):
    ...
    try:  
        cursor = conexion.cursor() # Creo cursor
        query = "DELETE FROM profesores WHERE idProfesor = %s" # Instrucción para eliminar un profesor
        cursor.execute(query, (dato,)) # Ejecutar la consulta con el parámetro seguro (con tuplas y no como caracteres)        
